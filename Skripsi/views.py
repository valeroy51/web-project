...
def admin_logout(request):
    ...
    print("USER:", request.user, request.user.is_authenticated, request.user.is_staff)
    
    refresh_token = request.data.get("refresh")

    if not refresh_token:
        return Response(
            {"status": "error", "message": "refresh token wajib dikirim"},
    ...
    try:
        ...
        return Response(
            {"status": "error", "message": "refresh token tidak valid"},
            status=status.HTTP_400_BAD_REQUEST
        )
...
def home(request):
    ...
    for data in station_data:
        ...
        realtime[status] = realtime.get(status, 0) + 1
...
def polutan_info(request):
    return JsonResponse({"status halaman": "ok", "page": "penjelasan"},status=200)
...
@api_view(["POST"])
@permission_classes([IsAdminUser])
def input_polutan(request):
    max_allowed_date = date.today() + timedelta(days=1)
    min_allowed_date = date(2022, 1, 1)

    polutan_form = PollutantDataForm(request.data if hasattr(request, "data") else request.POST)

    if not polutan_form.is_valid():
        return JsonResponse({
            "status": "error",
            "action": "save_pollutant",
            "errors": polutan_form.errors,
        }, status=400)
    ...
    if tanggal_input > max_allowed_date:
        return JsonResponse({
            "status": "error",
            "message": f"Tanggal {tanggal_input} melebihi {max_allowed_date}.",
        }, status=400)

    if tanggal_input < min_allowed_date:
        return JsonResponse({
            "status": "error",
            "message": f"Tanggal {tanggal_input} sebelum {min_allowed_date}.",
        }, status=400)
    ...
    return JsonResponse({
        ...
        "created": created,
        "message": "Data polutan baru disimpan." if created else "Data polutan lama diperbarui.",
        "station": getattr(station_input, "nama_stasiun", None),
        "tanggal": str(tanggal_input),
    }, status=200)

@api_view(["POST"])
@permission_classes([IsAdminUser])
def input_meteo(request):
    max_allowed_date = date.today() + timedelta(days=1)
    min_allowed_date = date(2022, 1, 1)

    meteo_form = MeteorologicalDataForm(request.data if hasattr(request, "data") else request.POST)

    if not meteo_form.is_valid():
        return JsonResponse({
            "status": "error",
            "action": "save_meteo",
            "errors": meteo_form.errors,
        }, status=400)
...
def upload_data(request):
    ...
    if not file:
        return JsonResponse({"status": "error", "message": "File tidak ditemukan. Kirim dengan key upload_file atau file."}, status=400)
    ...
    # normalize meteo cols using extract_short_code
    new_cols = []
    for col in df.columns:
        code = extract_short_code(col)
        new_cols.append(code if code in meteo_cols else col)
    df.columns = new_cols
    ...
    if pollutant_cols.intersection(df.columns):
        ...
        return JsonResponse({"status": "error", "message": f"Kolom file tidak dikenali: {list(df.columns)}"}, status=400)

    # simpan preprocessed (kalau kamu mau tetap)
    OUTPUT_PRE = os.path.join(settings.BASE_DIR, "Dataset", "Preprocessed Website")
    os.makedirs(OUTPUT_PRE, exist_ok=True)
    pre_path = os.path.join(OUTPUT_PRE, f"preprocessed_{os.path.splitext(uploaded_filename)[0]}.xlsx")
    df_clean.to_excel(pre_path, index=False)
    ...
    return JsonResponse({
        ...
        "inserted": inserted,
        "skipped": skipped,
        "preprocessed_saved_to": pre_path,
    }, status=200)
